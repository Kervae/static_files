...
$("#create-sub").click(function() {
	...
	var list = $('#list').prop('checked') ? "true": "false";
	console.log("list: " + list);
	if ($("#client option:selected").val().indexOf("surge") != -1) {
		var return_url = sub_api + "?target=" + target + "&ver=" + ver + "&url=" + encodeURIComponent(sub_link) + "&emoji=" + emoji + "&list=" + list + "&insert=false&config=" + config_url + "&include=" + encodeURIComponent($('#include').val()) + "&exclude=" + encodeURIComponent($('#exclude').val());
	} else {
		if (config !== "default") {
			var return_url = sub_api + "?target=" + target + "&url=" + encodeURIComponent(sub_link) + "&emoji=" + emoji + "&list=" + list + "&insert=false&config=" + config_url + "&include=" + encodeURIComponent($('#include').val()) + "&exclude=" + encodeURIComponent($('#exclude').val());
		} else {
			var return_url = sub_api + "?target=" + target + "&url=" + encodeURIComponent(sub_link) + "&emoji=" + emoji + "&list=" + list + "&insert=false&include=" + encodeURIComponent($('#include').val()) + "&exclude=" + encodeURIComponent($('#exclude').val());
		}
	}
